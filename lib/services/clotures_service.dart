...
class CloturesService {
  ...
  Stream<List<Cloture>> streamClotures(String espaceId) {
    final flux = _client.from('clotures').stream(primaryKey: ['id']).eq('espace_id', espaceId).order('date', ascending: false);
    return avecCacheHorsLigne('clotures_$espaceId', flux).map((rows) => rows.map(Cloture.fromMap).toList());
  }
