...
class MembresService {
  ...
  Stream<List<Membre>> streamMembres(String espaceId) {
    final flux = _client.from('membres').stream(primaryKey: ['id']).eq('espace_id', espaceId).order('nom');
    return avecCacheHorsLigne('membres_$espaceId', flux).map((rows) => rows.map(Membre.fromMap).toList());
  }
