...
class InvitationsService {
  ...
  Stream<List<Invitation>> streamMesInvitations(String email) {
    return _client
      ...
      final espaceIds = rows.map((r) => r['espace_id'] as String).toSet().toList();
      final espaces = await _client.from('espaces').select('id, nom').inFilter('id', espaceIds);
  ...
  Future<void> inviter(Invitation invitation) => _client.from('invitations').insert(invitation.toInsertMap());

  /// Annule (admin) ou refuse (invité) une invitation — même opération,
  /// c'est la RLS qui distingue qui a le droit de le faire.
  Future<void> supprimer(String invitationId) => _client.from('invitations').delete().eq('id', invitationId);
  ...
  Future<String> accepter(String invitationId) async {
    final espaceId = await _client.rpc('accepter_invitation', params: {'p_invitation_id': invitationId});
